...
Auth_MantisBT is an extension for MediaWiki 1.27 and up which logs-in
...
Copy this extension directory `Auth_MantisBT/` into your mediawiki
extension folder `extensions/`. Then add the following lines to your
global configuration file `LocalSettings.php`:

    $wgAuthMantisBTPath = '/<path to your mantisbt instance>/mantisbt' ;
    wfLoadExtension( 'Auth_MantisBT' );
...
`$wgAuthMantisBT`. Just add them to your `LocalSettings.php`. Default
...
* Set the name(s) to use for mapping into the local wiki user database.
  ...
  Examples:
        ...
        $wgAuthMantisBTUserName = null; // default
        ...
        $wgAuthMantisBTUserName = $_SERVER[ 'LOGON_USER' ];

        $wgAuthMantisBTUserName = ""; // Will evaluate to nothing.
        $wgAuthMantisBTUserName = []; // Will evaluate to nothing.

        // This is not advised, because it will evaluate every visitor
        // to the same wiki user 'Everybody'.
        $wgAuthMantisBTUserName = "Everybody";

        // Create a closure instead of providing strings directly.
        $wgAuthMantisBTUserName = function() {
...
* When you need to process your remote user name before it can be used
  ...
  blacklisting some names, use the hook `AuthMantisBTFilterUserName`
...
* This extension comes with predefined remote user name filters (which
  ...
  PCRE syntax too):
        ...
        $wgAuthMantisBTUserNameReplaceFilter = null; // default
...
* If you want to prevent some names from beeing logged in automatically,
  ...
  where each name can also be a regular expression in PCRE syntax:
        ...
        $wgAuthMantisBTUserNameBlacklistFilter = null; // default
...
* The opposite of the `UserNameBlacklistFilter` can be achieved by using
  ...
  preventing it:
        ...
        $wgAuthMantisBTUserNameWhitelistFilter = null; // default
...
* When you have further user information available in your environment,
  ...
  and disabling their modifiability:
        ...
        $wgAuthMantisBTUserPrefs = null;       // default
        $wgAuthMantisBTUserPrefsForced = null; // default
...
* You can replace urls in MediaWiki, if your remote source is better
  ...
  * `logout` - Provide a redirect url for the user logout. Depending on
  ...
  Examples:
        ...
        $wgAuthMantisBTUserUrls = null; // default
...
* By default this extension mimics the behaviour of Auth_remoteuser
  ...
  with the following configuration:
        ...
        $wgAuthMantisBTAllowUserSwitch = false; // default

        $wgAuthMantisBTAllowUserSwitch = true;
...
* As an immutable SessionProvider (see `AllowUserSwitch` config above)
  ...
  this one, then set the following accordingly:
        ...
        $wgAuthMantisBTRemoveAuthPagesAndLinks = true; // default

        $wgAuthMantisBTRemoveAuthPagesAndLinks = false;
...
* If you are using other SessionProvider extensions besides this one,
  you have to specify their significance by using an ascending priority:
        ...
        $wgAuthMantisBTPriority = 50; // default

        $wgAuthMantisBTPriority = SessionInfo::MAX_PRIORITY;
...
* `$wgAuthMantisBTAuthz` - This parameter has no equivalent new
  parameter, because you can achive the same with not loading the
  extension at all.
* `$wgAuthMantisBTName` - Superseded by `$wgRemoteuserUserPrefs`.
* `$wgAuthMantisBTMail` - Superseded by `$wgRemoteuserUserPrefs`.
* `$wgAuthMantisBTNotify` - Superseded by `$wgRemoteuserUserPrefs`.
* `$wgAuthMantisBTDomain` - Superseded by
  `$wgRemoteuserUserNameReplaceFilter`.
* `$wgAuthMantisBTMailDomain` - Superseded by `$wgRemoteuserUserPrefs`.
